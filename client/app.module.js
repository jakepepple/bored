"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
const platform_browser_1 = require('@angular/platform-browser');
const http_1 = require('@angular/http');
const core_1 = require('@angular/core');
const app_component_1 = require('./app.component');
const app_layout_component_1 = require('./app-layout.component');
const event_form_component_1 = require('./event-form.component');
const login_button_component_1 = require('./login-button.component');
const add_event_button_component_1 = require('./add-event-button.component');
const login_component_1 = require('./login.component');
const app_routes_1 = require('./app.routes');
let AppModule = class AppModule {
};
AppModule = __decorate([
    core_1.NgModule({
        imports: [
            platform_browser_1.BrowserModule,
            http_1.HttpModule,
            http_1.JsonpModule,
        ],
        declarations: [
            app_component_1.AppComponent,
            app_layout_component_1.AppLayoutComponent,
            event_form_component_1.EventFormComponent,
            login_component_1.LoginComponent,
            login_button_component_1.LoginButtonComponent,
            add_event_button_component_1.AddEventButtonComponent
        ],
        providers: [
            app_routes_1.APP_ROUTES_PROVIDER
        ],
        bootstrap: [app_component_1.AppComponent]
    }), 
    __metadata('design:paramtypes', [])
], AppModule);
exports.AppModule = AppModule;
//# sourceMappingURL=app.module.js.map